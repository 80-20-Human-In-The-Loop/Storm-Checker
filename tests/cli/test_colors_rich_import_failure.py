...
def test_colors_module_with_rich_import_failure():
    ...
    try:
        ...
        theme = cli.colors.get_rich_theme()
        assert theme is None
        
        # Test get_rich_color (line 334)
        color = cli.colors.get_rich_color("primary")
        assert color is None
        
        # Test create_rich_style (line 352)
        style = cli.colors.create_rich_style("primary")
        ...
        try:
            cli.colors.demo_rich_integration()
        ...
        sys.stdout = io.StringIO()
        try:
            cli.colors.print_rich_header("Test")
            output = sys.stdout.getvalue()
        ...
        result = cli.colors.EnhancedColorPrinter.rich_text("Test", "primary")
        assert "Test" in result
        
        result = cli.colors.EnhancedColorPrinter.rich_text("Test", "unknown")
        assert result == "Test"
...
def test_print_rich_functions_with_import_exceptions():
    ...
    # First ensure cli.colors is loaded normally
    ...
    for func_name, message, icon in functions_to_test:
        ...
        try:
            func = getattr(cli.colors, func_name)
    ...
    try:
        ...
        try:
            cli.colors.print_rich_header("Header", "Subtitle")
            output = sys.stdout.getvalue()
...
def test_demo_rich_integration_with_error():
    ...
    # Remove cli.colors if already loaded
    if 'cli.colors' in sys.modules:
        del sys.modules['cli.colors']
